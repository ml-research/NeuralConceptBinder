...
class NeSyConceptLearner(nn.Module):
    ...
    """
    def __init__(self, n_classes, n_set_heads=4, set_transf_hidden=128, args=None):
        """
        ...
        self.precompute_bind = args.precompute_bind
        # Concept Embedding Module
        self.img2state_net = NeuralConceptBinder(args)

        # get list of the indices from concatenated one hote encodings that belong to which category/property/factor
        self.category_ids = list(np.cumsum([len(torch.unique(self.img2state_net.retrieval_corpus[block_id]['ids']))
             for block_id in range(args.num_blocks)]))
        self.category_ids.insert(0, 0)

        one_hot_enc_size = np.sum(
            [len(torch.unique(self.img2state_net.retrieval_corpus[block_id]['ids']))
             for block_id in range(args.num_blocks)]
        )
        # Reasoning module
        self.set_cls = SetTransformer(dim_input=one_hot_enc_size, dim_hidden=set_transf_hidden, num_heads=n_set_heads,
    ...
    def forward(self, data):
        ...
        :param data: 4D Tensor [batch, channels, width, height]
        ...
        """
        # make sure either imgs were passed when precompute_bind is False otherwise data should be onehot encodings
        assert not self.precompute_bind and len(data.shape) == 4 or self.precompute_bind and len(data.shape) == 3
        # if the images were not already prepared via the retrieval binder, then pass img through ret binder
        # and transform the integers to one hot encodings
        if not self.precompute_bind:
            data, _ = self.img2state_net.encode(data)
            # binarize slot attention output, apart from coordinate output
            data = self._transform_attrs(data)
        # run through classifier via set transformer
        cls = self.set_cls(data)

        return cls.squeeze(), data
    ...
    def _transform_attrs(self, attrs):
        ...
        attrs_one_hot = [
            torch.nn.functional.one_hot(
                attrs[:, :, block_id].long(),
                num_classes=len(torch.unique(self.img2state_net.retrieval_corpus[block_id]['ids']))
            )

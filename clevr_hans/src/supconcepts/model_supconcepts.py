...
class SlotAttention(nn.Module):
    ...
    """
    def __init__(self, num_slots, dim, iters=3, eps=1e-8, hidden_dim=128, device='cuda'):
        super().__init__()
        ...
        self.slots_log_sigma = nn.Parameter(torch.randn(1, 1, dim)).abs().to(self.device)
...
class SlotAttention_model(nn.Module):
    ...
    def __init__(self, n_slots, n_iters, n_attr,
        ...
        self.slot_attention = SlotAttention(num_slots=n_slots, dim=encoder_hidden_channels, iters=n_iters, eps=1e-8,
                                            hidden_dim=attention_hidden_channels, device=device)
...
class NeSyConceptLearner(nn.Module):
    ...
    """
    def __init__(self, n_classes, n_slots=4, n_iters=3, n_attr=18, n_set_heads=4, set_transf_hidden=128,

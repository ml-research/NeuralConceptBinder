...
CUDA_VISIBLE_DEVICES=$DEVICE python train.py --data-dir $DATA --seed $SEED --dataset $DATASET --epochs 500 --name $MODEL --lr 0.0004 --batch-size 2 --n-slots 4 --n-iters-slot-att 3 --n-attr 18 --train-only --resume runs/slot-attention-clevr-state-0-0/slot-attention-clevr-state-0-0

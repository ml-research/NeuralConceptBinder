...
def perform_cluster_per_block_precomputed(data, block_id, args, reduce_dim=100, n_param_steps=10, verbose=0,
	...
	#TODO: currently only performing default parameter clustering, comment this in for parameter search
...
def gather_obj_encs(model, loader, args):
	...
	for i, sample in tqdm(enumerate(loader)):
		...
		# encode image
		slot_encs, _, attn_masks, _ = model.encode(imgs)

		block_encs = slots_to_blocks(slot_encs, args)

		attn_masks_plot = attn_masks

		if 'CLEVR' in args.data_path:
			obj_slot_ids = find_slot_id_with_obj(atattn_maskstns.detach().cpu().numpy(), args)
			...
			block_encs = torch.flatten(block_encs, start_dim=0, end_dim=1)
			all_encs.append(block_encs.detach().cpu().numpy())
			# multiply images with attention masks of slot with highest attention value
			# (remember we assert args.thresh_count_obj_slots == 0)
			imgs = imgs.repeat_interleave(args.num_slots, dim=0)
			attn_masks_plot = torch.flatten(attn_masks_plot, start_dim=0, end_dim=1)
			imgs = imgs * attn_masks_plot
			all_imgs.append(np.transpose(imgs.detach().cpu().numpy(), (0, 2, 3, 1)))
...
def main():
	...
	args.log_dir = os.path.sep.join([*args.checkpoint_path.split(os.path.sep)[:-1]])
	...
	print(f'{train_block_encs.shape} encodings for clustering!')
